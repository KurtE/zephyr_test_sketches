...
ST7735_zephyr::ST7735_zephyr(struct spi_dt_spec * pspi, const struct gpio_dt_spec *CS, const struct gpio_dt_spec *DC, const struct gpio_dt_spec *RST) :
...
{
  USBSerial.print("ST7735::setRotation ");
  USBSerial.print(m);
  USBSerial.print(" Tab: ");
  USBSerial.println(tabcolor);
  beginSPITransaction();
  writecommand_cont(ST77XX_MADCTL);
  ...
  //USBSerial.printf("SetRotation(%d) _xstart=%d _ystart=%d _width=%d, _height=%d\n", _rot, _xstart, _ystart, _width, _height);
