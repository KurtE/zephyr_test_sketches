...
ST7789_zephyr::ST7789_zephyr(struct spi_dt_spec * pspi, const struct gpio_dt_spec *CS, const struct gpio_dt_spec *DC, const struct gpio_dt_spec *RST) :
...
void  ST7789_zephyr::begin(uint16_t width, uint16_t height, uint32_t spi_clock)
{
  printk("ST7789 begin(%u, %u, %u)\n", width, height, spi_clock);
...
void  ST7789_zephyr::setRotation(uint8_t m) 
{
  ...
  endSPITransaction();
//  USBSerial.printf("Set _rotation %d start(%d %d) row: %d, col: %d\n", m, _xstart, _ystart, _rowstart, _colstart);
  setClipRect();
  setOrigin();
