...
#ifdef GRAY_IMAGE
uint8_t frame_buffer[GRAY_IMAGE_FUDGE_SIZE];
#else
...
int main(void)
{
	...
	// Quick and dirty try to initialize the RESET pin.
		pinMode(PA_9, OUTPUT);
		...
		delay(1);
		digitalWrite(PA_9, LOW);
		printk("GPIOA: MODER:%x AF:%x %x\n", GPIOA->MODER, GPIOA->AFR[0], GPIOA->AFR[1]);
	...
	printk("DCMI CR:%x SR:%x RISR:%x IER:%x\n", DCMI->CR, DCMI->SR, DCMI->RISR, DCMI->IER);
	printk("\tMISR:%x ESCR:%x ESUR:%x CWSTRT:%x CWSIZE:%x\n", DCMI->MISR, DCMI->ESCR, DCMI->ESUR, DCMI->CWSTRTR, DCMI->CWSIZER);
	// show all registers
//	show_all_gpio_regs();
	int frame_count = 0;
...
//		#endif
		...
		start_time = micros();
		uint8_t *pbIn = (uint8_t*)vbuf->buffer;
		uint8_t *pbOut = (uint8_t*)vbuf->buffer;
		for (uint32_t i = 0; i < vbuf->bytesused; i+=2) {
			uint8_t b =  ((pbIn[i] & 0xf) << 0) + ((pbIn[i + 1] & 0xf) << 4);
			*pbOut++ = b;
		}
		tft.writeRect8BPP(0, 0, CAMERA_IMAGE_WIDTH, CONFIG_VIDEO_FRAME_HEIGHT, pbIn, pallete);
    //uint16_t *pixels = (uint16_t *)vbuf->buffer;
		//tft.writeRect8BPP_4BITB(0, 0, CAMERA_IMAGE_WIDTH, CONFIG_VIDEO_FRAME_HEIGHT, 
		//		pixels, pallete);
		printk("writeRect8BPP: %u %u %d %lu\n", vbuf->size, vbuf->bytesused, frame_count, micros() - start_time);
...
//	WaitForUserInput(2000);
	#ifdef GRAY_IMAGE
	for (int i = 0; i < 256; i++) pallete[i] = ILI9341_GIGA_n::color565(i, i, i);
	uint16_t screen_width = tft.width();
	uint8_t gray_line[screen_width];
	for (int y = 0; y < 240; y++) {
		memset(gray_line, y+8, screen_width);
		tft.writeRect8BPP(0, y, screen_width, 1, gray_line, pallete);
	}
	k_sleep(K_MSEC(500));
	#endif
...
int initialize_video() {
	...
	fmt.pixelformat = VIDEO_PIX_FMT_GREY_44; //VIDEO_PIX_FMT_RGB565;
	...
	printk("Initialze video buffer list bs: %u (%u)\n",  bsize, ARRAY_SIZE(buffers));
